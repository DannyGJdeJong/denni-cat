...
export const StickerStats = (): React.ReactElement => {
  const packUsageQuery = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        new URL("/api/pack-usage/371151432834875396", API_URL).href
      );
  ...
  React.useEffect(() => {
    if (
      ...
      packUsageQuery.data.packUsage.total_usage !== packUsage
    ) {
  ...
  return (
    ...
    >
      {packUsageQuery.status === "success" ? (
        <NumberTicker
          startValue={oldPackUsage}
          endValue={packUsageQuery.data.packUsage.total_usage}
          textSize="min(16vw, 16rem)"
        />
      ) : (
        <></>
      )}
...
export const Route = createFileRoute("/sticker-stats/")({
  component: StickerStats,
});
